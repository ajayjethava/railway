...
def import_xlsx_data_to_database(xlsx_path, project_id, is_junction=False, junction_identifier_field='name'):
    """Import XLSX data into database tables - Only update specific junction if is_junction=True"""
    with app.app_context():
        try:
            logger.info(f"Importing XLSX data for project {project_id}, is_junction={is_junction}")
            ...
            if is_junction:
                # For junction files, we need to identify which junction to update
                logger.info(f"Processing as JUNCTION file - will try to update specific junction")
                print(f"🔄 Processing junction update - will try to identify junction")
                
                # Try to find junction identifier from junction_box sheet
                if 'junction_box' in wb.sheetnames:
                    try:
                        ws = wb['junction_box']
                        headers = [str(cell.value).strip().lower().replace(' ', '_') for cell in ws[1] if cell.value]
                        
                        # Look for common identifier fields
                        identifier_field = None
                        identifier_values = set()
                        
                        # First, check which identifier fields exist in the sheet
                        possible_id_fields = ['name', 'code', 'junction_name', 'junction_code', 
                                             'description', 'id', 'junction_id', 'box_name', 'box_code']
                        ...
                        # If no standard field found, use the first non-standard field
                        if not identifier_field and headers:
                            identifier_field = headers[0]
                        
                        if identifier_field:
                            print(f"🔍 Using '{identifier_field}' as junction identifier")
                            
                            # Collect all unique identifier values from the sheet
                            for row in ws.iter_rows(min_row=2, values_only=True):
                                for i, cell_value in enumerate(row):
                                    if i < len(headers) and headers[i] == identifier_field and cell_value is not None:
                                        identifier_values.add(str(cell_value).strip())
                            
                            # Delete only junctions with these identifier values
                            if identifier_values:
                                for identifier in identifier_values:
                                    if identifier:
                                        ...
                                        # Check if field exists in JunctionBox model
                                        if hasattr(JunctionBox, identifier_field):
                                            deleted_count = JunctionBox.query.filter_by(**filter_dict).delete()
                                            if deleted_count > 0:
                                                logger.info(f"Deleted {deleted_count} junctions with {identifier_field}='{identifier}'")
                                                print(f"🗑️  Deleted {deleted_count} junction(s) with {identifier_field}='{identifier}'")
                                            else:
                                                logger.info(f"No existing junctions found with {identifier_field}='{identifier}'")
                                                print(f"ℹ️  No existing junctions with {identifier_field}='{identifier}' (will add new)")
                                        else:
                                            logger.warning(f"Field '{identifier_field}' not found in JunctionBox model, skipping deletion")
                                            print(f"⚠️  Field '{identifier_field}' not in JunctionBox model, will add without deletion")
                                
                                db.session.commit()
                                print(f"✅ Junction-specific deletion complete")
                            else:
                                print(f"⚠️  No identifier values found in junction_box sheet")
                        else:
                            print(f"⚠️  Could not find identifier field in junction_box sheet")
                            
                    except Exception as e:
                        logger.error(f"Error processing junction file: {e}")
                        print(f"⚠️  Warning: Could not process junction identifier: {e}")
                        db.session.rollback()
                
                # For other tables, don't delete anything for junction files
                print(f"ℹ️  For junction files, other tables (Cable, Terminal, etc.) will not be cleared")
                
            else:
                # For non-junction files, delete all records as before
                logger.info(f"Processing as NON-JUNCTION file - will replace all data")
                try:
                    ...
                    db.session.commit()
                    logger.info(f"Deleted ALL existing records for project {project_id}")
                    print(f"🗑️  Deleted ALL old records for project {project_id}")
                except Exception as e:
                    db.session.rollback()
                    logger.error(f"Error deleting ALL existing records: {e}")
                    print(f"⚠️  Warning: Could not delete old records: {e}")
            
            # Import StationDrawing sheet
            ...
            # Import data from all other sheets
            tables_to_import = [
...
def process_remote_xlsx_file_enhanced(remote_filename):
    ...
    try:
        ...
        junction_num = junction_info['junction_number']
        ...
        if file_type == 'JUNCTION':
            print(f"🔌 Junction number from filename: {junction_num}")
        ...
        if file_type == 'JUNCTION':
            # Use junction number for naming
                ...
                if process_success:
                    ...
                    if upload_success:
                        ...
                        if db_success:
                            ...
                            # Import data with junction-specific handling
                            logger.info("Importing XLSX data to database tables...")
                            print("💿 Importing data to DB...")
                            if file_type == 'JUNCTION':
                                import_success = import_xlsx_data_to_database(
                                    local_xlsx_new, 
                                    project_id, 
                                    is_junction=True
                                )
                            ...
                            if import_success:
                                ...
                                if file_type == 'JUNCTION':
                                    print(f"🔌 JUNCTION IMPORT: Only updated specific junction data")
                                    print(f"🔌 Other junctions in the project were preserved")
                                elif file_type == 'PREVIEW':
                                    print(f"📭 No notifications sent for PREVIEW PDF")
                                else:  # FINAL
                                    print(f"📢 Notifications sent to level1 approvers WITH PROJECT ACCESS ONLY")
                            else:
                                print(f"⚠️  Warning: Data import had issues")
                    ...
                    if remote_success and CONFIG['ARCHIVE_REMOTE']:
                        ...
                        archive_results = bulk_remote_operations(archive_operations)
...
def monitor_xlsx_download_folder(flask_app=None):
    ...
    print("  6. JUNCTION file support with PRESERVATION of existing junctions")
    print("="*80)
    print("\n📂 DOWNLOAD LOCATIONS:")
    print("-"*40)
    ...
    print("📋 Junction handling:")
    print("   • When processing JUNCTION files, existing junctions are PRESERVED")
    print("   • Only the specific junction being updated is replaced")
    print("   • Other junctions in the project remain untouched")
    print("📋 Notifications:")
                ...
                if current_xlsx:
                    ...
                    for filename in current_xlsx:
                        ...
                        if success:
                            ...
                            if file_type == 'JUNCTION':
                                print(f"🔄 Project status kept as 'junction' for JUNCTION file")
                                print(f"🔌 Only updated specific junction - other junctions preserved")
...
if __name__ == "__main__":
    ...
    print("  • JUNCTION file support with PRESERVATION of existing junctions")
