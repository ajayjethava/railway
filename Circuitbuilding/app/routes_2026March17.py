...
def save_terminal_header_draft():
    ...
    try:
        ...
        for h in existing_headers:
            ...
            key = (header_type_clean, terminal_start_clean, terminal_end_clean)
...
def delete_projects(project_id):
    ...
    try:
        ...
        # IMPORTANT: First, delete junction_box_summary records to avoid NULL constraint violation
        try:
            # Try using raw SQL to delete from junction_box_summary
            db.session.execute(
                text("DELETE FROM junction_box_summary WHERE project_id = :project_id"),
                {'project_id': project_id}
            )
            print(f"Deleted records from junction_box_summary for project {project_id}")
        except Exception as e:
            print(f"Warning when deleting from junction_box_summary: {e}")
            # If the table doesn't exist or SQL fails, try to handle it gracefully
            try:
                # Try alternative: set project_id to a valid value (like 0) if we can't delete
                # But better to delete if possible
                db.session.execute(
                    text("UPDATE junction_box_summary SET project_id = 0 WHERE project_id = :project_id"),
                    {'project_id': project_id}
                )
                print(f"Updated junction_box_summary records for project {project_id}")
            except:
                print(f"Could not handle junction_box_summary for project {project_id}")
...
def download_ctr_sample_template():
    ...
    try:
        ...
        rowdetail_data = [
            ["A", 18, "SP", "2403", 1, 18, 2, ""],
            ["A", 2, "12T B/N24V", "2403", 19, 20, 2, "pink"],
            ["A", 4, "POT FR", "2403", 21, 24, 2, "yellow"],
            ["A", 4, "BX110V", "2401P", 1, 4, 2, "yellow"],
            ["A", 4, "NX110V", "2401P", 5, 8, 2, "yellow"]
        ]
...
def upload_ctr_xlsx():
    ...
    try:
        ...
        try:
            ...
            def generate_pdf_background(upload_id, file_path, flask_app, upload_data):
                ...
                try:
                    ...
                    with flask_app.app_context():
                        ...
                        try:
                            ...
                            ctr_upload = session.query(CTRUpload).get(upload_id)
                            if not ctr_upload:
