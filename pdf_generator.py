#!/usr/bin/env python3
"""
Railway Project Terminal Diagram Generator - PDF Generation Module
Updated with font fixes, robust error handling, and comprehensive logging
"""
...
# === LOGGING SETUP ===
def setup_logging():
    """Set up comprehensive logging for PDF generator"""
    logger = logging.getLogger("railway_project.pdf_generator")
    ...
    log_file = os.path.join(LOG_DIR, "pdf_generator.log")
...
def store_pdf_metadata(db_conn, pdf_metadata):
    ...
    try:
            ...
            for pattern in patterns:
                ...
                if match:
                    try:
                        ...
                        logger.info(f"Extracted project_id: {project_id} from filename: {xlsx_filename}")
                        break
                    except ValueError:
                        continue
        ...
        if project_id is None:
                ...
                if numbers:
                    try:
                        for num in numbers:
                            if len(num) >= 2:
                                project_id = int(num)
                                logger.info(f"Using fallback project_id: {project_id} from numbers in filename")
                                break
                    except ValueError:
                        pass
            ...
            if project_id is None:
                project_id = 0
                logger.warning(f"Could not extract project_id from filename. Using default: {project_id}")
        ...
        db_conn.commit()
        cursor.close()
        logger.info(f"PDF metadata stored in database with project_id: {project_id}")
        return True
        
    except Exception as e:
        logger.error(f"Error storing PDF metadata in database: {e}")
        logger.error(traceback.format_exc())
        db_conn.rollback()
        return False
...
def generate_pdf_metadata_checksum(df_title, excel_file_path, pdf_file_path):
    ...
    try:
        ...
        logger.debug(f"Checksum data: {checksum_data}")
...
def enhance_pdf_with_metadata(pdf_file_path, checksum, checksum_data, content_size, df_title):
    try:
        ...
        try:
            ...
            logger.warning(f"Could not read PDF with PyPDF2: {e}")
            logger.warning("Skipping metadata enhancement as PDF may be corrupted or in use")
            return False
        ...
        try:
            ...
            logger.error(f"Error writing enhanced PDF: {e}")
            logger.error(traceback.format_exc())
            return False
        ...
        try:
            ...
            logger.info(f"PDF metadata enhanced with checksum: {checksum}")
            return True
        except PermissionError:
            logger.warning("Cannot replace the original PDF because it is open or locked. The enhanced PDF is saved as '" + temp_output + "'")
            return False
        except Exception as e:
            logger.error(f"Error replacing original PDF: {e}")
            logger.error(traceback.format_exc())
            return False
    except Exception as e:
        logger.error(f"Error enhancing PDF metadata: {e}")
        logger.error(traceback.format_exc())
        return False
...
def update_pdf_checksum_metadata(pdf_file_path, checksum, checksum_data, content_size, df_title):
    ...
    try:
        ...
        logger.debug(f"Initial file size: {initial_content_size} bytes")
        ...
        logger.debug(f"Initial checksum with initial file size: {updated_checksum}")
        logger.debug(f"Checksum data string: {updated_checksum_data}")
        ...
        logger.debug(f"Final file size after enhancement: {final_content_size} bytes")
        ...
        if final_content_size != initial_content_size:
            ...
            updated_checksum = hashlib.md5(updated_checksum_data.encode()).hexdigest()
            logger.debug(f"Updated checksum with final file size: {updated_checksum}")
            logger.debug(f"Updated checksum data string: {updated_checksum_data}")
        
        # Compute full MD5 of the entire PDF file content
        try:
            with open(pdf_file_path, 'rb') as f:
                full_file_md5 = hashlib.md5(f.read()).hexdigest()
            logger.debug(f"Full file MD5 hash: {full_file_md5}")
        except Exception as e:
            logger.warning(f"Could not compute full file MD5: {e}")
            full_file_md5 = "N/A"
        
        # Store as log file
        log_file = os.path.join(LOG_DIR, 'checksum.log')
        try:
            with open(log_file, 'a') as f:
                f.write("\n" + "-" * 50 + "\n")
                f.write(f"Timestamp: {timestamp}\n")
                f.write(f"File: {os.path.basename(pdf_file_path)}\n")
                f.write(f"Station code: {station_code}\n")
                f.write(f"Metadata Checksum: {updated_checksum}\n")
                f.write(f"Full file MD5: {full_file_md5}\n")
                f.write(f"Initial size: {initial_content_size} bytes\n")
                f.write(f"Final size: {final_content_size} bytes\n")
            logger.debug(f"Checksum details appended to '{log_file}'")
        except Exception as e:
            logger.warning(f"Could not write to checksum log: {e}")
        
        return updated_checksum, updated_checksum_data, final_content_size, full_file_md5, timestamp, initial_content_size
    except Exception as e:
        logger.error(f"Error updating PDF metadata: {e}")
        logger.error(traceback.format_exc())
...
def get_row_order(letter):
    ...
    try:
        ...
        logger.error(f"Error in get_row_order for letter '{letter}': {e}")
        return 0
...
def break_cables_into_rows_updated(cable_list, max_terminal_symbols_per_row=36, max_cable_boxes_per_row=6):
    ...
    try:
        ...
        logger.error(f"Error in break_cables_into_rows_updated: {e}")
        logger.error(traceback.format_exc())
        return []
...
def draw_cable_box_row(ax, x_start, y_center, cable_info, pin_spacing=0.8):
    ...
    try:
        ...
        logger.error(f"Error in draw_cable_box_row: {e}")
        logger.error(traceback.format_exc())
        return [], [], []
...
def draw_extra_connections(ax, cable_rows, x_positions, terminal_nos_for_positions,
    ...
    try:
        ...
        for _, row in cable_rows.iterrows():
            ...
            if pd.notna(extra_input) and str(extra_input).strip() != '':
                try:
                    ...
                    logger.error(f"Error processing input_connected_extra '{extra_input}': {e}")
                    logger.error(traceback.format_exc())
            ...
            if pd.notna(extra_output) and str(extra_output).strip() != '':
                try:
                    ...
                    logger.error(f"Error processing output_connected_extra '{extra_output}': {e}")
                    logger.error(traceback.format_exc())
        ...
        logger.error(f"Error in draw_extra_connections: {e}")
        logger.error(traceback.format_exc())
...
def merge_ranges(ranges, merge_adjacent=True):
    ...
    try:
        ...
        logger.error(f"Error in merge_ranges: {e}")
        logger.error(traceback.format_exc())
        return []
...
def get_block_cable_name(df_block):
    try:
        ...
        logger.error(f"Error in get_block_cable_name: {e}")
        logger.error(traceback.format_exc())
        return ""
...
def draw_cable_name(ax, x, y, cable_name, x_offset=0.65):
    try:
        ...
        logger.error(f"Error in draw_cable_name: {e}")
        logger.error(traceback.format_exc())
...
def draw_junction_box(ax, x, y, junction_name, rect_pad=0.2):
    try:
        ...
        logger.error(f"Error in draw_junction_box: {e}")
        logger.error(traceback.format_exc())
...
def draw_relay_input(ax, x_left, x_right, y=0, scale=1.0, text='RELAY', anchor_to_v_tip=False, v_offset=-0.5, is_not_connected_with_bus=False):
    try:
        ...
        logger.error(f"Error in draw_relay_input: {e}")
        logger.error(traceback.format_exc())
...
def draw_relay_output(ax, x_left, x_right, y=0, scale=1.0, text='RELAY', anchor_to_v_tip=False, v_offset=0.5, is_not_connected_with_bus=False):
    try:
        ...
        logger.error(f"Error in draw_relay_output: {e}")
        logger.error(traceback.format_exc())
...
def draw_group_top_symbol(
    ...
    try:
        ...
        logger.error(f"Error in draw_group_top_symbol: {e}")
        logger.error(traceback.format_exc())
...
def draw_group_bottom_symbol(
    ...
    try:
        ...
        logger.error(f"Error in draw_group_bottom_symbol: {e}")
        logger.error(traceback.format_exc())
...
def draw_relay_box_top(ax, x_start, x_end, y, texts='R1', scale=1.0, input_connected='N', spacing=0.3, x_offset=0.3):
    ...
    try:
        ...
        logger.error(f"Error in draw_relay_box_top: {e}")
        logger.error(traceback.format_exc())
...
def draw_relay_box_bottom(ax, x_start, x_end, y, texts='R1', scale=1.0,
    ...
    try:
        ...
        logger.error(f"Error in draw_relay_box_bottom: {e}")
        logger.error(traceback.format_exc())
...
def draw_relay_contact_box_top(ax, x_start, x_end, y, texts='R1', scale=1.0, input_connected='N', spacing=0.3, x_offset=0.3):
    ...
    try:
        ...
        logger.error(f"Error in draw_relay_contact_box_top: {e}")
        logger.error(traceback.format_exc())
...
def draw_relay_contact_box_bottom(ax, x_start, x_end, y, texts='R1', scale=1.0,
    ...
    try:
        ...
        logger.error(f"Error in draw_relay_contact_box_bottom: {e}")
        logger.error(traceback.format_exc())
...
def draw_header(ax, cable_id, header_type, x_start, x_end, text, min_symbol_bottom=None,
    ...
    try:
        ...
        logger.error(f"Error in draw_header: {e}")
        logger.error(traceback.format_exc())
...
def find_row_by_term(term):
    try:
        ...
        logger.error(f"Error in find_row_by_term: {e}")
        logger.error(traceback.format_exc())
        return None
...
def draw_capsule(ax, x, y_center, terminal_no, input_left, input_right, output_left, output_right,
    ...
    try:
        ...
        logger.error(f"Error in draw_capsule: {e}")
        logger.error(traceback.format_exc())
...
def draw_s_fuse(ax, x, y_center, terminal_no,
    ...
    try:
        ...
        logger.error(f"Error in draw_s_fuse: {e}")
        logger.error(traceback.format_exc())
        return (x, y_center), (x, y_center), 'N', 'N'
...
def draw_choke(ax, x, y_center, terminal_no):
    try:
        ...
        logger.error(f"Error in draw_choke: {e}")
        logger.error(traceback.format_exc())
        return (x, y_center), (x, y_center), 'N', 'N'
...
def draw_horizontal_choke(ax, x_center, y_center, label='CHOKE',
    ...
    try:
        ...
        logger.error(f"Error in draw_horizontal_choke: {e}")
        logger.error(traceback.format_exc())
        return None
...
def draw_dual_fuse(ax, x_left, y_center, left_term, right_term, left_input_left=None, left_input_right=None, left_output_left=None, left_output_right=None, left_input_connected='N', left_output_connected='N', right_input_left=None, right_input_right=None, right_output_left=None, right_output_right=None, right_input_connected='N', right_output_connected='N'):
    try:
        ...
        logger.error(f"Error in draw_dual_fuse: {e}")
        logger.error(traceback.format_exc())
...
def draw_resistor(ax, x, y_center, input_terminal='', output_terminal='', resistor_name='R', input_x_pos=None, output_x_pos=None):
    try:
        ...
        logger.error(f"Error in draw_resistor: {e}")
        logger.error(traceback.format_exc())
        return None, None
...
def draw_rectangle_symbol(ax, x, y_center, terminal_no, symbol, input_left, input_right, output_left, output_right,
    ...
    try:
        ...
        logger.error(f"Error in draw_rectangle_symbol: {e}")
        logger.error(traceback.format_exc())
        return (x, y_center), (x, y_center), 'N', 'N'
...
def draw_input_connection(ax, x, symbol_top_y, input_connected_flag, output_connected_flag, y_top_bus_group, has_bus_line=True):
    ...
    try:
        ...
        logger.error(f"Error in draw_input_connection: {e}")
        logger.error(traceback.format_exc())
...
def draw_output_connection(ax, x, symbol_bottom_y, input_connected_flag, output_connected_flag, y_bottom_bus_group, has_bus_line=True):
    ...
    try:
        ...
        logger.error(f"Error in draw_output_connection: {e}")
        logger.error(traceback.format_exc())
...
def draw_bus_lines(ax, x_positions, connected_flags, bus_y, gap=0.12, extra=0.12, y_offset=0.15):
    try:
        ...
        logger.error(f"Error in draw_bus_lines: {e}")
        logger.error(traceback.format_exc())
...
def parse_terminal_no_field(val):
    try:
        ...
        logger.error(f"Error in parse_terminal_no_field: {e}")
        logger.error(traceback.format_exc())
        return None, None
...
def draw_horizontal_choke_updated(ax, x_center, y_center, label='CHOKE',
    ...
    try:
        ...
        logger.error(f"Error in draw_horizontal_choke_updated: {e}")
        logger.error(traceback.format_exc())
        return None
...
def draw_symbols(df, ax, page_rows, junction_name, start_x=1, pin_spacing=0.8, cables_per_page=12, page_number=1, max_terminal_symbols_per_row=36, max_rows_visible=3, page_width=None, global_max_width=None):
    ...
    try:
        ...
        logger.debug(f"Processing {len(page_rows)} rows for this page")
    
        for row_index, (letter, cable_list) in enumerate(page_rows):
            logger.debug(f"Processing row {row_index + 1} - Letter '{letter}' with {len(cable_list)} cables")
            ...
            for cable_id in cable_list:
                ...
                if is_cable_box:
                    ...
                    if cable_type == 'relay_box':
                        ...
                        if current_terminal_count + position_num > max_terminal_symbols_per_row:
                            logger.error(f"Cable boxes in row '{letter}' exceed row limit")
                            continue
                ...
                if not cable_rows.empty:
                    ...
                    logger.debug(f"Cable {cable_id} requires {symbols_to_add_total} terminal symbols")
                
                    if current_terminal_count + symbols_to_add_total > max_terminal_symbols_per_row:
                        logger.error(f"Cable {cable_id} would exceed row limit")
                        continue
                    ...
                    if not resistor_row.empty:
                        ...
                        if current_terminal_count + symbols_to_add > max_terminal_symbols_per_row:
                            logger.warning(f"Resistor would exceed row limit")
                    ...
                    logger.debug(f"Cable {cable_id} has {len(cable_headers_temp)} header(s)")
                    ...
                    for _, hrow_temp in cable_headers_temp.iterrows():
                        ...
                        logger.debug(f"Header: type={header_type_temp}, start={start_name_temp}, end={end_name_temp}")
                       
                        if pd.isna(start_name_temp) or pd.isna(end_name_temp):
                            logger.debug(f"Skipping: start or end is None")
                            continue
                           
                        if start_name_temp not in terminal_nos_for_positions:
                            logger.warning(f"start_name '{start_name_temp}' not found in terminal_nos_for_positions")
                            if '.' in start_name_temp:
                                ...
                                if start_name_temp_no_dec in terminal_nos_for_positions:
                                    ...
                                    logger.debug(f"Found without decimal: {start_name_temp}")
                                else:
                                    continue
                            else:
                                continue
                        ...
                        if end_name_temp not in terminal_nos_for_positions:
                            logger.warning(f"end_name '{end_name_temp}' not found in terminal_nos_for_positions")
                            if '.' in end_name_temp:
                                ...
                                if end_name_temp_no_dec in terminal_nos_for_positions:
                                    ...
                                    logger.debug(f"Found without decimal: {end_name_temp}")
                                else:
                                    continue
                            else:
                                continue
                        ...
                        logger.debug(f"Adding range: {start_idx_temp} to {end_idx_temp} for {header_type_temp}")
                    ...
                    logger.debug(f"Top ranges before merge: {top_ranges}")
                    logger.debug(f"Bottom ranges before merge: {bottom_ranges}")
                    ...
                    logger.debug(f"Top segments after merge: {top_segments}")
                    logger.debug(f"Bottom segments after merge: {bottom_segments}")
                   
                    if not top_segments and any(input_connected_flags):
                        top_segments = [(0, len(x_positions)-1)]
                        logger.debug(f"Created default top segment: {top_segments}")
                    if not bottom_segments and any(output_connected_flags):
                        bottom_segments = [(0, len(x_positions)-1)]
                        logger.debug(f"Created default bottom segment: {bottom_segments}")
                   
                    logger.debug(f"Final top segments: {top_segments}")
                    logger.debug(f"Final bottom segments: {bottom_segments}")
        ...
        logger.error(f"Error in draw_symbols: {e}")
        logger.error(traceback.format_exc())
        return [], [], []
...
def draw_footer(ax, left, right, fixed_ylim_min, total_pages, page_num, df_title_row, junction_name):
    ...
    try:
        ...
        logger.error(f"Error in draw_footer: {e}")
        logger.error(traceback.format_exc())
...
def organize_junction_rows(junction_cables_regular, junction_cables_box):
    ...
    try:
        ...
        logger.error(f"Error in organize_junction_rows: {e}")
        logger.error(traceback.format_exc())
        return OrderedDict()
...
def create_pages_for_junction(junction, letter_groups, max_rows_per_page=3):
    ...
    try:
        ...
        logger.info(f"All letters in junction: {sorted_letters}")
        logger.info(f"Letter groups: {[(letter, len(cables)) for letter, cables in letter_groups.items()]}")
        ...
        logger.error(f"Error in create_pages_for_junction for junction '{junction}': {e}")
        logger.error(traceback.format_exc())
        return []

# === MAIN PDF GENERATION FUNCTION ===
def generate_pdf_from_excel(excel_file_path, output_filename=None):
    """
    Main function to generate PDF from Excel file
    """
    try:
        logger.info("="*60)
        logger.info(f"PDF GENERATION STARTED FOR: {excel_file_path}")
        logger.info("="*60)
        
        excel_filename = os.path.basename(excel_file_path)
        logger.info(f"Processing Excel file: {excel_filename}")

        # Validate required sheets exist
        try:
            xls = pd.ExcelFile(excel_file_path)
        except Exception as e:
            logger.error(f"Unable to open Excel file: {e}")
        ...
        if missing:
            logger.error(f"Excel file is missing required sheets: {missing}")
        ...
        try:
            logger.debug("Loading cable sheet...")
            df_cable = pd.read_excel(excel_file_path, sheet_name='cable')
            df_cable.columns = df_cable.columns.str.strip()
            
            try:
                logger.debug("Loading cable_box sheet...")
                df_cable_box = pd.read_excel(excel_file_path, sheet_name='cable_box')
                df_cable_box.columns = df_cable_box.columns.str.strip()
                logger.info("Loaded cable boxes from 'cable_box' sheet")
            except Exception as e:
                logger.warning(f"No 'cable_box' sheet found: {e}. Creating empty cable_box DataFrame.")
            ...
            logger.error(f"Error reading cable sheets from Excel file: {e}")
            return False, "Error reading cable sheets"

        # Load other required sheets
        try:
            logger.debug("Loading other required sheets...")
            ...
            try:
                ...
                logger.info("Loaded StationDrawing sheet for footer.")
            except Exception as e:
                logger.warning(f"Could not load StationDrawing sheet for footer: {e}. Footer will be skipped.")
                df_title = None
                
        except Exception as e:
            logger.error(f"Error reading required sheets from Excel file: {e}")
        ...
        if 'spare' in df.columns:
            try:
                ...
                logger.warning(f"Error processing spare column: {e}")

        # Prepare Plotting
        df_symbols = df.reset_index(drop=True)
        logger.info(f"Loaded {len(df_symbols)} symbols from terminal sheet")

        # Safety check for df_cable_box
        if df_cable_box is None or df_cable_box.empty:
            logger.warning("No cable box data found. Creating empty DataFrame with required structure.")
        ...
        try:
            ...
            logger.info(f"Processed {len(df_cable)} cables")
            ...
            if not df_cable_box.empty and 'row' in df_cable_box.columns:
                ...
                logger.info(f"Processed {len(df_cable_box)} cable boxes")
        except Exception as e:
            logger.error(f"Error preparing cable data: {e}")

        # Get unique junction names
        junction_names = pd.unique(df_cable['junction_name'].astype(str).str.strip())
        logger.info(f"Found {len(junction_names)} junctions: {list(junction_names)}")
        ...
        for junction in junction_names:
            try:
                ...
                logger.info(f"\n=== Junction: {junction} ===")
                logger.info("Cable order for width calculation:")
                for cable_id in cable_list:
                    ...
                    logger.info(f"  Cable {cable_id} ({cable_type}): Row '{cable_row.get('row', '')}', Position {cable_row.get('position', '')}")
                ...
                logger.debug(f"Junction '{junction}' max width: {max_row_width}")
            except Exception as e:
                logger.error(f"Error computing row widths for junction '{junction}': {e}")
                junction_row_widths[junction] = 30.0

        global_max_width = max(junction_row_widths.values()) + 2.0 if junction_row_widths else 30.0
        logger.info(f"Global max width for PDF: {global_max_width}")
        ...
        for junction in junction_names:
            try:
                ...
                if not df_cable_box.empty and 'junction_name' in df_cable_box.columns:
                    ...
                    logger.info(f"No cable box data for junction: {junction}")
                
                if junction_cables_regular.empty and junction_cables_box.empty:
                    logger.warning(f"No data found for junction '{junction}'. Skipping.")
                    continue
                ...
                logger.error(f"Error processing junction '{junction}': {e}")
                continue
        ...
        for i, (junction, page_rows) in enumerate(pages, 1):
            ...
            logger.info(f"Page {i}: {junction} - Rows: {row_info}")
        ...
        if os.path.exists(output_file):
            logger.warning(f"'{output_file}' already exists. Adding checksum suffix.")
        ...
        try:
            with PdfPages(output_file) as pdf:
                for page_num, (junction_name, page_rows) in enumerate(pages, 1):
                    try:
                        ...
                        plt.close(fig)
                        logger.info(f"Page {page_num} (Junction: {junction_name}) added to '{output_file}'")
                    except Exception as e:
                        logger.error(f"Error processing page {page_num} for junction '{junction_name}': {e}")
                        continue
            ...
            logger.info(f"PDF checksum finalized: {final_checksum}")
        except Exception as e:
            logger.error(f"Error creating PDF: {e}")
            return False, "Error creating PDF"

        logger.info(f"Multi-page PDF saved as '{output_file}'")
        ...
        try:
            ...
            logger.warning(f"Error enhancing PDF metadata: {e}")
        ...
        try:
            ...
            if db_conn:
                ...
                db_conn.close()
                logger.info("PDF metadata successfully stored in database.")
            else:
                logger.warning("Could not connect to database. Skipping database storage.")
        except Exception as e:
            logger.error(f"Error storing PDF metadata in database: {e}")

        logger.info("=" * 60)
        logger.info("PDF GENERATION COMPLETED SUCCESSFULLY!")
        logger.info(f"Input Excel: {excel_filename}")
        logger.info(f"Output PDF: {output_filename}")
        logger.info(f"PDF saved to: {output_file}")
        logger.info(f"Total pages generated: {total_pages}")
        logger.info("=" * 60)
        
        return True, output_file
        
    except Exception as e:
        logger.critical(f"CRITICAL ERROR: An unexpected error occurred in PDF generation: {e}")
        return False, str(e)
...
if __name__ == "__main__":
    # Command line usage
    if len(sys.argv) < 2:
        print("Usage: python pdf_generator.py <excel_file_path> [output_filename]")
        sys.exit(1)
    ...
    success, result = generate_pdf_from_excel(excel_file_path, output_filename)
    if success:
