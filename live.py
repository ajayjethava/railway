...
# === CONFIGURATION ===
XLSX_INPUT_DIR = "/root/srv/railway/frontend/xlsx_download"
PDF_OUTPUT_DIR = "/root/srv/railway/frontend/uploads"
PROCESSED_EXCEL_DIR = "/root/srv/railway/frontend/processed_excel"
ERROR_EXCEL_DIR = "/root/srv/railway/frontend/error_excel"
...
def store_pdf_metadata(db_conn, pdf_metadata):
    ...
    Expected columns:
    id, project_id, pdf_filename, xlsx_filename, file_size, checksum_md5,
    metadata_checksum, metadata_data, initial_size_bytes, final_size_bytes,
    metadata_ts_ist, station_code, source_pdf_name, full_file_md5, remarks,
    checksum_algo, created_at, level1_status, level2_status, level3_status,
    level2_approver_id, level3_approver_id, level2_approval_date,
    level3_approval_date, rejection_reason, level1_approver_id,
    level1_approval_date, version, junction_data
    """
    try:
        cursor = db_conn.cursor()
        
        # Parse project_id from xlsx_filename - FIXED VERSION
        ...
        if xlsx_filename:
            ...
            patterns = [
                # Pattern 1: RAILWAYPROJECT_ID55_test01_20260123_104038.xlsx
                r'RAILWAYPROJECT_ID(\d+)',
                # Pattern 2: PROJECT_49_kll_JUNCTION_4_A2_F_20260122_141945.xlsx  
                r'PROJECT_(\d+)',
                # Pattern 3: project_123_filename.xlsx (case insensitive)
                r'(?i)project[_-](\d+)',
                # Pattern 4: ID123_filename.xlsx
                r'ID(\d+)',
                # Pattern 5: Any number after underscore
                r'_(\d+)_'
            ]
        ...
        if project_id is None:
            # Check if project_id is in metadata
            ...
            if project_id is None:
                ...
                if numbers:
                    try:
                        # Use the first significant number (skip small numbers like timestamps)
                        for num in numbers:
                            if len(num) >= 2:  # Skip single digits or small numbers
                                project_id = int(num)
            ...
            # If still None, set to a default value (0 or -1 depending on your DB schema)
            # Check your database schema - if project_id can be 0 or has a default, use that
            if project_id is None:
                project_id = 0  # Or whatever default your database accepts
        ...
        # SQL query - Updated to handle NULL project_id if needed
        # First check if we should insert with project_id or without it
        if project_id is not None:
            ...
            values = (
                ...
                pdf_metadata.get('pdf_filename'),  # source_pdf_name same as pdf_filename
                pdf_metadata.get('full_file_md5'),
                'Generated by Python script',
                'md5',
                datetime.now(timezone('Asia/Kolkata')).strftime("%Y-%m-%d %H:%M:%S.%f"),
                'pending',  # level1_status
                'pending',  # level2_status
                'pending',  # level3_status
                1,  # version
                junction_data_json
            )
        else:
            # If your database schema allows NULL for project_id, use this query
            insert_query = """
            INSERT INTO generated_pdf(
            ...
            # Prepare values without project_id
            values = (
                pdf_metadata.get('pdf_filename'),
                xlsx_filename,
                pdf_metadata.get('final_size_bytes'),
...
def draw_group_bottom_symbol(
    ...
    try:
        ...
        if choke_output_terminal is None:
            ...
            # Horizontal line
            # Small vertical line BEFORE horizontal line (extend upward)
            x_shift_left = 0.005 * scale # move to the left
            small_vertical = 0.95 * scale
            ax.plot(
                [start_x - x_shift_left, start_x - x_shift_left], # shifted left
                [line_y, line_y - small_vertical], # downward
                color='black',
                linewidth=1.2
            )
            # Horizontal line
            ...
            ax.text(text_x, text_y, display_text, ha='center', va='top',
                    fontsize=int(21 * scale), fontname='Arial', linespacing=1.2)
    except Exception as e:
...
def draw_relay_box_bottom(ax, x_start, x_end, y, texts='R1', scale=1.0,
    ...
    try:
        ...
        # YOU used this for rectangle shift â€” keeping as-is
                    ...
                    for choke_idx, choke_row in choke_rows.iterrows():
                        ...
                        if input_term and input_term in terminal_nos_for_positions:
                            ...
                            if output_term and output_term in terminal_nos_for_positions and output_type != 'relay':
                                ...
                                # If output is connected (existing logic) â€” keep your previous behavior
                                elif output_connected_terminals:
...
def process_excel_file(excel_file_path):
    ...
    This is the main processing function that was previously in the main try-catch block
    """
    try:
        print("\n" + "="*60)
        ...
        try:
            ...
            try:
                df_cable_box = pd.read_excel(excel_file_path, sheet_name='relay_box')
        ...
        for junction in junction_names:
            junction_data.append({
                ...
                "junction_size": "Full",  # Default value, adjust as needed
        ...
        # Use the original Excel filename as base
        output_base = 'Terminal_Symbols_Centered_Fixed_Size'
        # Remove .xlsx extension and add timestamp
        excel_basename = os.path.splitext(excel_filename)[0]
        # Generate timestamp suffix in IST for uniqueness
        ist_tz = timezone('Asia/Kolkata')
        current_time = datetime.now(ist_tz)
                ...
                for page_num, (junction_name, page_rows) in enumerate(pages, 1):
                    try:
                        ...
                        x_positions, input_connected_flags, output_connected_flags = draw_symbols(
                            ...
                            global_max_width=global_max_width  # ADD THIS LINE
                        )
        ...
        try:
            ...
            if db_conn:
                # Prepare PDF metadata for database
                pdf_metadata = {
...
def monitor_directory():
    ...
    This function runs 24x7 until shutdown signal is received
    """
    print("\n" + "="*60)
    ...
    # Create a set to track files being processed to avoid duplicates
    processed_files = set()
        ...
        try:
            ...
            if excel_files:
                ...
                for excel_file in excel_files:
                    ...
                    # Check if file is still being written (try to open exclusively)
                    try:
                        # Try to open the file in read-write mode to check if it's accessible
                        with open(excel_path, 'rb+'):
                            pass
                    except IOError:
                        # File is still being written or locked, skip for now
                    ...
                    # Move Excel file after processing
                    if success:
