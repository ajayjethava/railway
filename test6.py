...
try:
    # === CONFIGURATION ===
    XLSX_INPUT_DIR = "/Users/admin/Documents/test/git1/git/temp_xlsx"
    ...
    # === LOAD LATEST XLSX FILE FROM DOWNLOAD FOLDER ===
    print(f"Looking for XLSX files in: {XLSX_INPUT_DIR}")
    EXCEL_FILE = get_latest_xlsx_file(XLSX_INPUT_DIR)

    if not EXCEL_FILE:
        print(f"No XLSX files found in {XLSX_INPUT_DIR}. Exiting.")
        sys.exit(1)
    ...
    try:
        ...
        try:
            df_cable_box = pd.read_excel(EXCEL_FILE, sheet_name='relay_box')
    ...
    print(f"CRITICAL ERROR: An unexpected error occurred in the main execution: {e}")
    traceback.print_exc()
    sys.exit(1)