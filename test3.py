...
def check_project_status(project_id):
    """Check if project has status 'ready_for_pdf' OR 'preview' - Process BOTH"""
    with app.app_context():
        try:
            ...
            if hasattr(project, 'status'):
                ...
                # Check if status is 'ready_for_pdf' OR 'preview' - PROCESS BOTH
                if current_status in ['ready_for_pdf', 'preview']:
...
def update_project_status(project_id, new_status, new_stage=None, is_preview=False):
    """Update project status and stage in database - Handle preview specially"""
    with app.app_context():
        try:
            project = Project.query.get(project_id)
            if not project:
...
def update_database_with_models(project_id, xlsx_filename, pdf_filename, checksum, metadata, station_info, version, is_preview=False):
    ...
    with app.app_context():
        try:
            ...
            # Check if this is a preview or final
            pdf_type = 'PREVIEW' if is_preview else 'FINAL'
            remarks = f'Auto-generated {pdf_type} PDF from XLSX'
            ...
            # Only create notifications for final PDFs, not for previews
            if not is_preview:
                logger.info("Creating notifications for new PDF (FINAL)...")
                create_pdf_notifications_ftp(generated_pdf, project, station_name, version)
            else:
                logger.info("Skipping notifications for PREVIEW PDF...")
...
def process_remote_xlsx_file_enhanced(remote_filename):
    ...
    try:
        ...
        # Check project status - now accepts both 'ready_for_pdf' and 'preview'
        ...
        if not project_ready:
            logger.info(f"Project {project_id} is not 'ready_for_pdf' or 'preview'. Skipping file: {remote_filename}")
            print(f"⏸️  Project {project_id} not ready_for_pdf or preview. Skipping: {remote_filename}")
            return False
        
        print(f"✅ Project {project_id} status is '{current_status}' - Proceeding...")
        print(f"📊 Processing as: {'PREVIEW' if is_preview else 'FINAL'}")
        ...
        # Add PREVIEW prefix for preview files
        if is_preview:
            ...
            if not checksum:
                ...
                if process_success:
                    ...
                    if upload_success:
                        ...
                        if db_success:
                            # Update project status based on whether it's preview or final
                            if is_preview:
                    ...
                    print(f"Type: {'PREVIEW' if is_preview else 'FINAL'}")
                    print("="*70)
...
def scan_and_process_remote():
    ...
    try:
        ...
        logger.info(f"Found {len(xlsx_files)} new XLSX file(s) to process (ready_for_pdf OR preview)")
        print(f"📊 New XLSX to process (ready_for_pdf OR preview): {len(xlsx_files)}")
...
def monitor_xlsx_download_folder(flask_app=None):
    ...
    print("  5. Processes BOTH 'ready_for_pdf' AND 'preview' status files")
    print("="*80)
    print("\n📂 DOWNLOAD LOCATIONS:")
    print("-"*40)
    ...
    print("="*80)
    print("📋 Monitoring for new XLSX files where project status = 'ready_for_pdf' OR 'preview'...")
    print("📋 Preview files will be marked with 'PREVIEW_' prefix")
    print("📋 Only FINAL files will trigger notifications")
    ...
    try:
        while True:
            ...
            if current_files:
                ...
                if current_xlsx:
                    logger.info(f"Found {len(current_xlsx)} new file(s) with ready_for_pdf or preview status")
                    print(f"\n📊 New files detected (ready_for_pdf OR preview): {len(current_xlsx)}")
...
if __name__ == "__main__":
    ...
    print("  • Processes projects with status = 'ready_for_pdf' OR 'preview'")
