...
# === Relay input symbol ===
def draw_relay_input(ax, x_left, x_right, y=0, scale=1.0, text='RELAY', anchor_to_v_tip=False,
                     v_offset=-0.5, is_not_connected_with_bus=False, multi_line_spacing=0.5):
    """
    Draw relay input symbol with text above.

    multi_line_spacing : factor that controls vertical gap between two lines
                         (gap = text_y_offset * multi_line_spacing).
                         Default 0.5 gives a very tight gap; increase for more space.
    """
    ...
    # For long text (≥8 characters), split into two lines and shift whole block upward
    if len(display_text) >= 8:
        long_text_extra_offset = 0.35 * scale   # moves entire block higher above symbol
        adjusted_text_y = text_y + long_text_extra_offset

        words = display_text.split()
        if len(words) > 1:
            line1 = " ".join(words[:-1])
            line2 = words[-1]
            # First line at adjusted reference y
            ax.text(center, adjusted_text_y, line1,
                    ha='center', va='bottom',
                    fontsize=int(21 * scale), fontname='Arial')
            # Second line below with reduced gap controlled by multi_line_spacing
            ax.text(center, adjusted_text_y - (text_y_offset * multi_line_spacing), line2,
                    ha='center', va='top',
                    fontsize=int(21 * scale), fontname='Arial')
        else:
            # Single long word split in middle
            mid = len(display_text) // 2
            line1 = display_text[:mid]
            line2 = display_text[mid:]
            ax.text(center, adjusted_text_y, line1,
                    ha='center', va='bottom',
                    fontsize=int(21 * scale), fontname='Arial')
            ax.text(center, adjusted_text_y - (text_y_offset * multi_line_spacing), line2,
...
def draw_relay_output(ax, x_left, x_right, y=0, scale=1.0, text='RELAY', anchor_to_v_tip=False, v_offset=0.5, is_not_connected_with_bus=False):
    ...
    # === UNCOMMENTED: Relay output graphical elements ===
    ax.plot([left_wire_x_start - end_slant_size, left_wire_x_start],
            [y + end_slant_size - v_offset, y - v_offset], color='black', linewidth=1.4)
    ax.plot([right_wire_x_end, right_wire_x_end + end_slant_size],
            [y - v_offset, y + end_slant_size - v_offset], color='black', linewidth=1.4)
    ...
    # REMOVE the early return for long text and instead handle it properly
    if len(display_text) >= 8:
        words = display_text.split()
        if len(words) > 1:
            line1 = " ".join(words[:-1])
            line2 = words[-1]
            # Draw first line
            ax.text(center, text_y, line1,
                   ha='center', va='top',
                   fontsize=int(21 * scale),  # CHANGED from 39 to 21
                   fontname='Arial')
            # Draw second line below
            ax.text(center, text_y - (text_y_offset * 2), line2,
                   ha='center', va='top',
                   fontsize=int(21 * scale),  # CHANGED from 39 to 21
                   fontname='Arial')
        else:
            # For single long word, split in middle
            mid = len(display_text) // 2
            line1 = display_text[:mid]
            line2 = display_text[mid:]
            # Draw first line
    ...
    # === UNCOMMENTED: Small vertical lines for "LIGHT" ===
...
def draw_group_top_symbol(
    ...
    auto_wrap=True,
    min_font_scale=0.8,
    is_line_short=False   # <-- NEW PARAMETER: True when the input connection line is short (Scenario 2)
):
    """
    ...
    If is_line_short is True, the text is shifted slightly downward.
    """
    ...
    for i in range(num_texts):
        ...
        # Extra downward shift when the connection line is short (Scenario 2)
        # Increased from 0.1 to 0.15 for a more noticeable drop
        extra_down = 0.15 * scale if is_line_short else 0.0
        
        # --- DETERMINE POSITION BASED ON TEXT LENGTH ---
        if len(display_text) < 4:
            # Less than 4 characters: Just a little above the symbol
            text_y = right_y + (0.18 * scale) - extra_down
            ...
            text_y = right_y + (0.45 * scale) - extra_down
        ...
        if auto_wrap and len(display_text) > max_chars_per_line:
            ...
            if len(display_text) >= 4:
                adjusted_text_y = text_y + (0.05 * scale)
            else:
...
def draw_rectangle_symbol(ax, x, y_center, terminal_no, symbol, input_left, input_right, output_left, output_right,
    ...
    if pd.notna(output_left) and str(output_left).strip() != "":
        ax.text(x - output_left_offset, y_center - rect_height / 2 - 0.15, format_text(output_left),
                fontsize=19, ha='right', va='top', rotation=90, linespacing=1.2, fontname='Arial')
    output_right_offset = 0.05
    if pd.notna( ) and str(output_right).strip() != "":
...
def draw_symbols(df, ax, page_rows, junction_name, start_x=1, pin_spacing=0.8, cables_per_page=12, page_number=1, max_terminal_symbols_per_row=36, max_rows_visible=3, page_width=None):
    ...
    for row_index, (letter, cable_list) in enumerate(page_rows):
        ...
        box_left_edges = []
        box_right_edges = []
        rect_height = 4   # same as inside function
        rect_width = 0.5
        
        # === UPDATED: Process ALL letters for cable boxes ===
        for cable_id in sorted(cable_list):
            ...
            if is_cable_box:
                ...
                if cable_type == 'relay_box':
                    ...
                    center_x = current_x + 1.0
                    rect_x = center_x - rect_width / 2
                    
                    box_left_edges.append(rect_x)
                    box_right_edges.append(rect_x + rect_width)
                    
                    current_x += rect_width + CABLE_GAP
            ...
            if not cable_rows.empty:
                ...
                #group = df_symbols[df_symbols['cable_id'] == cable_id].sort_index().reset_index(drop=True)
                print("CABLE zzzzzzzzz:", cable_id)
                print(df_symbols[df_symbols['cable_id'] == cable_id][['terminal_no']])
                group = df_symbols[df_symbols['cable_id'] == cable_id] \
                    .sort_values('terminal_no') \
                ...
                group.columns = group.columns.str.lower()
                group.columns = group.columns.str.replace(" ", "_")

                print("Columns:", list(group.columns))   # DEBUG
                '''
                if "terminal_start" in group.columns and "terminal_end" in group.columns:
                    group = group.sort_values(by=["terminal_start", "terminal_end"])
                else:
                    print("⚠ terminal columns not found")
                '''
                #print("Group Data")
                #print(group)
                while i < len(group):
                ...
                for key, texts in relay_top.items():
                    ...
                    draw_group_top_symbol(
                        ax, x_left, x_right, vertical_line_start,
                        texts=texts, scale=1.0,
                        input_connected='Y' if input_conn_flag else 'N',
                        is_line_short=is_not_connected_with_bus   # <-- PASS THE FLAG
                    )
        ...
        if box_left_edges:
            ...
            outer_bottom = capsule_y_center - rect_height/2 - 0.9
            outer_top = capsule_y_center + rect_height/2 + 0.9
...
def organize_junction_rows(junction_cables_regular, junction_cables_box):
    ...
    # Ensure numeric
    junction_cables['cable_id'] = pd.to_numeric(junction_cables['cable_id'], errors='coerce')

    # ✅ STEP 1: Get UNIQUE cables in correct order
    cable_order = (
        junction_cables
        .sort_values(by=['cable_id'], ascending=[True])
        ['cable_id']
        .drop_duplicates()
        .tolist()
    )

    # ✅ STEP 2: Map each cable to its row
    cable_to_row = (
        junction_cables
        .drop_duplicates(subset=['cable_id'])
        .set_index('cable_id')['row']
        .to_dict()
    )

    # ✅ STEP 3: Build groups correctly
...
def organize_junction_rows_old(junction_cables_regular, junction_cables_box):
    """
    ...
    junction_cables = junction_cables.sort_values(
        ...
        ascending=[False, True]  # Row descending, position descending
    )
...
if len(sys.argv) > 2 and sys.argv[2].strip():
    output_base = sys.argv[2]  # Use provided output path
else:
    # Default: save in uploads folder with timestamp
    UPLOADS_DIR = r"C:\Railway\git\uploads" #'/var/www/html/git/uploads'
    os.makedirs(UPLOADS_DIR, exist_ok=True)

    excel_name = os.path.basename(EXCEL_FILE).replace('.xlsx', '')
   
    
    ist_tz = timezone('Asia/Kolkata')
    timestamp_suffix = datetime.now(ist_tz).strftime("%Y-%m-%d_%H-%M-%S")
    output_base = os.path.join(UPLOADS_DIR, f"{excel_name}.pdf")
# Generate timestamp suffix in UTC for uniqueness
ist_tz = timezone('Asia/Kolkata')
current_time = datetime.now(ist_tz)
timestamp_suffix = current_time.strftime("%Y-%m-%d_%H-%M-%S")
output_file = sys.argv[2]  #output_base # f"{output_base}_{timestamp_suffix}.pdf"
    ...
    for page_num, (junction_name, page_rows) in enumerate(pages, 1):
        ...
        for letter, cable_list in page_rows:
            for cid in cable_list:
                ...
                if is_cable_box:
                    r = r_box
                    row_letter  = r['row'].iloc[0] if not r.empty and 'row' in r.columns else ""
                else:
                    r = df_cable[df_cable['cable_id'] == cid]
                    row_letter  = r['row'].iloc[0] if not r.empty and 'row' in r.columns else ""
                    
                if row_letter  != current_letter and current_terminal_count_page > 0:
                ...
                group = df_symbols[df_symbols['cable_id'] == cid] \
                    .sort_values('terminal_no') \
                    .reset_index(drop=True)
                total_terminals = 0
                added_width = 0
